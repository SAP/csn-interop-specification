...
for (const file of files) {
  ...
  for (const definitionName in csnModel.definitions) {
    csnModel.definitions[definitionName] = fix(
      csnModel.definitions[definitionName],
      file,
    );
    for (const elementName in csnModel.definitions[definitionName]) {
      csnModel.definitions[definitionName][elementName] = fix(
        csnModel.definitions[definitionName][elementName],
        file,
      );
    }
  }
...
function fix(csn, fileName) {
  if (
    csn["@ObjectModel.supportedCapabilities"] &&
    !Array.isArray(csn["@ObjectModel.supportedCapabilities"])
  ) {
    csn["@ObjectModel.supportedCapabilities"] = [
      csn["@ObjectModel.supportedCapabilities"],
    ];
    console.log(
      `Fixed ${fileName}: Converted @ObjectModel.supportedCapabilities to array.`,
    );
  }
